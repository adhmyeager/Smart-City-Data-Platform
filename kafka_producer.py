    ...
    def _connect(self, retries: int = 5) -> None:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                wait = 2 ** attempt
                logger.warning(f"[Kafka] No broker (attempt {attempt}/{retries}), retry in {wait}s …")
                time.sleep(wait)
...
def run(dry_run: bool = False, verbose: bool = False) -> None:
    ...
    while running:
        ...
        # ── 1. Weather every 5 min ────────────────────────────
        if now_ts - last_weather_fetch >= WEATHER_INTERVAL:
            try:
                weather = fetch_weather("cairo")
                fleet.inject_weather(weather)
                producer.send(TOPICS["weather"], weather.to_dict(), key="cairo")
                stats.weather_sent += 1
                last_weather_fetch  = now_ts
                logger.info(f"[Weather] {weather.condition} {weather.temp_c}°C (source={weather.source})")
            except Exception as e:
        ...
        for reading in readings:
            ...
            producer.send(TOPICS["telemetry"], payload, key=reading.vehicle_id)
        ...
        # ── 3. Traffic every 60 sec ───────────────────────────
        # FIX: استخدم latitude و longitude الحقيقيين من الـ vehicle
        # (الكود القديم كان بيبعت engine_temp كـ latitude — خطأ!)
        if now_ts - last_traffic_fetch >= TRAFFIC_INTERVAL:
            for vehicle in fleet.vehicles:
                try:
                    t = fetch_traffic(
                        lat=vehicle.latitude,   # ← GPS الحقيقي
                        lon=vehicle.longitude,  # ← GPS الحقيقي
                    )
                    producer.send(TOPICS["traffic"], t.to_dict())
                except Exception as e:
                    logger.error(f"[Traffic] Fetch error for {vehicle.vehicle_id}: {e}")
